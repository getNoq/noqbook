...
export function WhoOwesMe() {
  ...
  const { invoices, isLoading, error, page, totalPages, sort, goToPage, changeSort } = useOwedInvoices();
      ...
      <main className="flex-1 min-w-0 px-4 md:px-8 py-6 md:py-8 mb-20">
        <div className="flex flex-col  min-[369px]:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm" style={{ color: BRAND.inkSoft }}>Every sale that isn't fully paid yet, in one place.</p>
          </div>
          {totalOwed !== null && totalOwed > 0 && (
            <div className="rounded-2xl px-5 py-3 text-left min-[369px]:text-right shrink-0" style={{ background: BRAND.peach }}>
              <div className="text-xs font-semibold uppercase tracking-wide" style={{ color: BRAND.red }}>Total owed</div>
              <div className="font-heading text-2xl" style={{ color: BRAND.red }}>₦{totalOwed.toLocaleString("en-NG")}</div>
            </div>
          )}
        </div>

        <div className="flex gap-2 mb-6 mt-5">
          <button onClick={() => changeSort("oldest")} className="rounded-full px-4 py-1.5 text-xs font-semibold" style={{ background: sort === "oldest" ? BRAND.ink : BRAND.card, color: sort === "oldest" ? BRAND.bg : BRAND.inkSoft, border: `1px solid ${BRAND.line}` }}>
            Oldest first
          </button>
          <button onClick={() => changeSort("largest")} className="rounded-full px-4 py-1.5 text-xs font-semibold" style={{ background: sort === "largest" ? BRAND.ink : BRAND.card, color: sort === "largest" ? BRAND.bg : BRAND.inkSoft, border: `1px solid ${BRAND.line}` }}>
            Largest balance
          </button>
        </div>
