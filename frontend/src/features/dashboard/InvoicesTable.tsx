...
export function InvoicesTable({ invoices, onSendReminder, onShareAsImage, onShareLink }: InvoicesTableProps) {
  ...
  return (
    <>
      <div className="flex flex-col gap-3 md:hidden">
        {invoices.map((inv) => (
          <InvoiceCard key={inv.id} invoice={inv} onSendReminder={() => onSendReminder(inv)} onShareAsImage={() => onShareAsImage(inv)} onShareLink={() => onShareLink(inv)} />
        ))}
      </div>
              ...
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} style={{ borderBottom: `1px solid ${BRAND.line}` }}>
                    ...
                    <td className="px-5 py-4">
                      <RowActionsMenu invoice={inv} onSendReminder={() => onSendReminder(inv)} onShareAsImage={() => onShareAsImage(inv)} onShareLink={() => onShareLink(inv)} />
                    </td>
                  </tr>
                );
              })}
